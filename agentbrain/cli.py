...
def cmd_check(a) -> int:
    ...
    print(
        f"graphify   : {'installed' if graph.graphify_available() else 'not installed (optional)'}"
    )
...
def cmd_scan(a) -> int:
    ...
    if not srcs:
        ...
        return 1
    normalize.run(v, srcs, cfg.get("max_msg_chars", 4000), cfg.get("max_note_chars", 60000))
    recall.cmd_index(v)
    return 0
...
def cmd_search(a) -> int:
    ...
    if not hits:
        ...
        print(
            "Absence here does not prove it never happened - the current session is never indexed."
        )
        return 0
...
def cmd_graph(a) -> int:
    ...
    if not ok:
        ...
        print(
            "Run:  python -m agentbrain.cli consent --accept   (or --local-only)", file=sys.stderr
        )
        return 3
...
def cmd_consent(a) -> int:
    ...
    if a.revoke:
        consent.revoke()
        print("Consent revoked. You will be asked again.")
        return 0
    if a.local_only:
...
def cmd_models(a) -> int:
    ...
    for r in ranked:
        print(
            f"{r['id']:<46}{r['in']:>8.3f}{r['out']:>8.3f}"
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap = argparse.ArgumentParser(
        prog="agentbrain", description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter
    )
    ...
    e.add_argument("format", choices=sorted(data.EXPORTERS))
    ...
    if a.cmd == "export":
        print(f"exported -> {data.EXPORTERS[a.format](_vault(a), a.out)}")
        return 0
    if a.cmd == "delete":
        return _delete(a)
    if a.cmd == "schedule":
        ok, msg = {
            "install": lambda: scheduler.install(_vault(a), a.hour),
