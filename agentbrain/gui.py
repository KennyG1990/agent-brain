...
class Tip:
    ...
    def _show(self, _e=None):
        ...
        t.wm_geometry(
            f"+{self.w.winfo_rootx() + 12}+{self.w.winfo_rooty() + self.w.winfo_height() + 6}"
        )
        tk.Label(
            t,
            text=self.text,
...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        nb.add(self.tab_graph, text="Graph  ($)")
        ...
        self.log = scrolledtext.ScrolledText(
            lf, height=8, bg="#0f1115", fg=DIM, font=("Consolas", 9), relief="flat", wrap="none"
        )
    ...
    def _home(self):
        f = self.tab_home
        box = ttk.Labelframe(f, text=" state ")
        box.pack(fill="x", padx=8, pady=8)
        self.lbl = {}
        for k, cap in (
            ("sources", "agents found"),
            ("notes", "conversations"),
            ("index", "search index"),
        ...
        tk.Label(
            box,
            text="green = fine  ·  amber = degraded  ·  red = broken",
            bg=BG,
            fg=DIM,
            font=("Segoe UI", 8),
            anchor="w",
        ...
        Tip(
            self.b_scan,
            "Reads your saved sessions from Claude Code, Cowork, Codex and "
            "Gemini, turns each conversation into a note, and rebuilds the "
            "search index.\n\nEntirely local. Nothing is uploaded. Free.",
        )
        ttk.Button(
            bar, text="Open folder", command=lambda: webbrowser.open(self.vault.as_uri())
        ).pack(side="left", padx=6)

        sf = ttk.Labelframe(f, text=" ask your history ")
        ...
        self.results = scrolledtext.ScrolledText(
            sf, height=12, bg="#0f1115", fg=FG, font=("Consolas", 9), relief="flat", wrap="word"
        )
    ...
    def _graph(self):
        ...
        tk.Label(
            f,
            text="The graph is OPTIONAL and is the only thing that costs money or "
        ...
        for c in cols:
            ...
            self.tree.column(
                c,
                width=w[c],
                anchor=("w" if c in ("fit", "model") else "e"),
                stretch=(c == "model"),
            )
        ...
        ttk.Button(r2, text="Preview (spends nothing)", command=self.do_preview).pack(
            side="left", padx=12
        )
        self.b_graph = tk.Button(
            r2,
            text="BUILD GRAPH  ($)",
            command=self.do_graph,
            bg="#5a2020",
            fg="#ffdede",
            relief="flat",
            padx=12,
        ...
        self.minfo = scrolledtext.ScrolledText(
            f, height=8, bg="#0f1115", fg=FG, font=("Consolas", 9), relief="flat", wrap="word"
        )
        self.minfo.pack(fill="both", expand=True, padx=10, pady=(0, 10))
        self.minfo.insert(
            "1.0",
            "Click 'Load models' to rank every OpenRouter model for this "
    ...
    def _settings(self):
        ...
        ttk.Checkbutton(
            ...
            command=lambda: self.key_e.config(show="" if self.show_key.get() else "•"),
        ...
        self.src_box = scrolledtext.ScrolledText(
            sf, height=8, bg="#0f1115", fg=FG, font=("Consolas", 9), relief="flat"
        )
        self.src_box.pack(fill="both", expand=True, padx=10, pady=8)
        ttk.Button(sf, text="Add a folder manually…", command=self.add_source).pack(
        ...
        ttk.Button(r3, text="Enable daily 3am", command=lambda: self.sched(True)).pack(
            side="right", padx=6
        )
    ...
    def _data(self):
        ...
        for fmt, label in (
            ...
            ttk.Button(r, text=label, command=lambda x=fmt: self.do_export(x)).pack(
                side="left", padx=(0, 8)
            )

        cf = ttk.Labelframe(f, text=" privacy ")
        cf.pack(fill="x", padx=8, pady=8)
        self.consent_lbl = tk.Label(
            cf,
            text="…",
            bg=BG,
            fg=FG,
        ...
        ttk.Button(r2, text="Review the disclosure", command=self.show_consent).pack(side="left")
        ttk.Button(r2, text="Switch to local-only", command=self.go_local).pack(side="left", padx=6)

        df = ttk.Labelframe(f, text=" delete ")
        df.pack(fill="both", expand=True, padx=8, pady=8)
        self.del_box = scrolledtext.ScrolledText(
            df, height=9, bg="#0f1115", fg=FG, font=("Consolas", 9), relief="flat"
        )
        self.del_box.pack(fill="both", expand=True, padx=10, pady=8)
        r3 = ttk.Frame(df)
        r3.pack(fill="x", padx=10, pady=(0, 10))
        ttk.Button(r3, text="What would be deleted?", command=self.show_delete_plan).pack(
            side="left"
        )
        tk.Button(
            r3,
    ...
    def show_consent(self, first: bool = False):
        ...
        t.insert("1.0", consent.SUMMARY)
        t.config(state="disabled")
        r = ttk.Frame(w)
        r.pack(fill="x", padx=10, pady=(0, 12))

        def choose(accept, local):
            consent.record(accepted=accept, local_only=local)
            self._w(
                "ok",
                f"privacy choice recorded: "
        ...
        ttk.Button(
            r,
            text="See a sample of what would be sent",
            command=lambda: messagebox.showinfo("Sample", consent.sample_upload(self.vault)),
        ).pack(side="left")
        ttk.Button(r, text="Local only - never upload", command=lambda: choose(False, True)).pack(
            side="right", padx=6
        )
        ttk.Button(
            r,
            text="I understand - allow uploads for the graph",
    ...
    def do_scan(self):
        def job(log):
            ...
            if not srcs:
                log("No agent session logs found on this computer.")
                return
            normalize.run(
                self.vault,
                srcs,
                self.cfg.get("max_msg_chars", 4000),
                self.cfg.get("max_note_chars", 60000),
                log=log,
            )
    ...
    def do_search(self):
        ...
        try:
            ...
            self.results.insert("1.0", "No index yet - click Scan My Computer.")
            return
        except ValueError as e:
            self.results.insert("1.0", str(e))
            return
        if not hits:
            self.results.insert(
                "1.0",
                f"No matches across {total} conversations.\n\n"
    ...
    def do_preview(self):
        ...
        cost = (
            f"~${est / 1e6 * a['in'] + est * 0.45 / 1e6 * a['out']:.2f}"
            if a and not a.get("free")
            else "unknown until you load models"
        )
        messagebox.showinfo(
            "Preview - nothing sent, nothing spent",
    ...
    def _refill(self):
        ...
        k = self._sortk
        rows.sort(
            key=lambda a: a[k] if not isinstance(a[k], str) else a[k].lower(), reverse=self._sortd
        )
        ...
        for a in rows:
            ...
            self.tree.insert(
                ...
                values=(
                    "OK" if good else ("!!" if not a["structured"] else "~"),
                    a["id"],
                    "free" if a["free"] else f"{a['in']:.3f}",
                    "free" if a["free"] else f"{a['out']:.3f}",
                    f"{a['ctx']:,}",
                    "yes" if a["thinks"] else "no",
    ...
    def do_export(self, fmt):
        if fmt == "obsidian":
            ...
            out = filedialog.asksaveasfilename(
                title="Export to…", defaultextension=".json" if fmt == "json" else ".md"
            )
        if not out:
            return
    ...
    def show_delete_plan(self):
        ...
        if not plan:
            self.del_box.insert("1.0", "Nothing stored - nothing to delete.")
            return
    ...
    def refresh(self):
        ...
        for k in self.lbl:
            ...
            txt = d.get("detail", "?")
            self.lbl[k].config(
                text=txt, fg=BAD if not d.get("ok", True) else (WARN if "⚠" in txt else OK)
            )
        nx = snap["next"]
        ...
        self.sched_lbl.config(
            text=f"{scheduler.backend()}: {'enabled' if okk else 'not scheduled'}"
        )
