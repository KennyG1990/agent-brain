...
def _post(url: str, headers: dict[str, str], payload: dict[str, Any], log=None) -> dict:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        if log:
            log(
                f"    retry {attempt}/{MAX_ATTEMPTS - 1} in {wait:.0f}s ({last.__class__.__name__})"
            )
...
class Meter:
    ...
    def __init__(
        self, usd_per_m_in: float = 0.0, usd_per_m_out: float = 0.0, cap_usd: float = 0.0
    ) -> None:
    ...
    def line(self) -> str:
        ...
        return f"{self.calls} calls · {self.tin:,} in / {self.tout:,} out · ${self.usd:.3f}{cap}"


def chat(
    cfg: dict, system: str, user: str, max_tokens: int = 2000, meter: Meter | None = None, log=None
) -> str:
...
def test_key(cfg: dict) -> tuple[bool, str]:
    ...
    if not cfg["api_key"] and "localhost" not in cfg["base_url"]:
        return False, (
            f"No API key set. Paste one in Settings, or set the "
