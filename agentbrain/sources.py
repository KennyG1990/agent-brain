...
def candidates() -> list[Source]:
    ...
    out: list[Source] = [
        ...
        Source("Codex", KIND_ROLLOUT_CODEX, h / ".codex" / "sessions", glob="rollout-*.jsonl"),
        Source(
            "Gemini / Antigravity", KIND_GEMINI_BRAIN, h / ".gemini" / "antigravity-ide" / "brain"
        ),
        Source(
            "Gemini / Antigravity (alt)", KIND_GEMINI_BRAIN, h / ".gemini" / "antigravity" / "brain"
        ),
    ]
    ...
    if sys.platform == "win32":
        ...
        if ad:
            out.append(
                Source(
                    "Claude Cowork", KIND_JSONL_CLAUDE, ad / "Claude" / "local-agent-mode-sessions"
                )
            )
        ...
        out.append(
            Source(
                "Claude Cowork",
                KIND_JSONL_CLAUDE,
                h / "Library" / "Application Support" / "Claude" / "local-agent-mode-sessions",
            )
        )
    else:
        out.append(
    ...
    try:
        if src.kind == KIND_GEMINI_BRAIN:
            for conv in src.path.iterdir():
                ...
                if (logs / "transcript_full.jsonl").exists() or (
...
def summary(extra: list[str] | None = None) -> str:
    ...
    if not live:
        return (
            "No AI agent session logs found on this computer. "
...
if __name__ == "__main__":  # quick probe: python -m agentbrain.sources
    for s in discover():
        ...
        print(
            f"{mark} {s.name:<28} {s.files:>5} files   {s.path}"
