...
class TestSecretsNeverLeak(Sandboxed):
    ...
    def test_refuses_to_persist_without_a_keystore(self):
        ...
        found = [
            ...
            if p.is_file() and "SHOULDNOTPERSIST" in p.read_text(encoding="utf-8", errors="ignore")
        ]
...
class TestNormalize(unittest.TestCase):
    ...
    def test_harness_scaffolding_is_stripped(self):
        ...
        self.assertEqual(
            strip_scaffold("<recommended_plugins>\nlots\n</recommended_plugins>\nreal question"),
            "real question",
        )
        self.assertEqual(
            strip_scaffold("# AGENTS.md instructions\n<INSTRUCTIONS>\nrules\n</INSTRUCTIONS>\nask"),
            "ask",
        )
        self.assertEqual(
            strip_scaffold(
...
class TestModelRanking(unittest.TestCase):
    ...
    def test_budget_scales_with_output_ceiling(self):
        ...
        self.assertGreater(
            mp.suggest_token_budget(big)["budget"], mp.suggest_token_budget(small)["budget"]
        )
...
class TestMCPServer(unittest.TestCase):
    ...
    def test_handshake_and_tool_list(self):
        ...
        self.assertEqual(
            {t["name"] for t in r[1]["result"]["tools"]}, {"recall", "read_note", "brain_stats"}
        )
    ...
    def test_path_traversal_is_refused(self):
        for evil in ("../../etc/passwd", "..\\..\\Windows\\System32\\config\\SAM", "/etc/shadow"):
            r = self._run(
                [
                    {
                        "jsonrpc": "2.0",
